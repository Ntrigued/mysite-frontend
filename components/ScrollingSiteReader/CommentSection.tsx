...
export default function CommentSection(props: any) {
  ...
  const is_visible: Boolean =
    props.is_visible == null ? false : props.is_visible;
  ...
  useEffect(() => {
    if (is_visible && comment_data.length == 0) {
      adapter
        .getComments(comment_ids)
        .then((data: Array<any>) => {
          setCommentData(data);
          setCommentOpenedList(Array(data.length).fill(false));
        })
        .catch((err: Error) =>
          console.error("There was an error getting comment info: ", err),
        );
    }
    ...
    .map((item: any, idx) => {
      ...
      return (
        <div
          key={item_id}
          className={
            "flex flex-col bg-white pt-[3vh] pb-[1.5vh] " +
        ...
        >
          ...
          >
            <div
              className={
                "flex flex-row justify-start ml-[2.5%] min-w-[15%] font-bold"
              }
            >
              {username}
            </div>
            <div
              className={
                " flex flex-col justify-start py-[1vh] pl-[1.25%] whitespace-pre-wrap"
              }
            ...
            >
              <div
                className={
                  "flex flex-row justify-start ml-[2.5%] min-w-[15%] text-sm"
                }
              >
                {datetime}
              </div>
              ...
              <div
                className={
                  "flex flex-row grow justify-end mr-[2.5%] italic text-sm"
                }
              >
                ({child_ids.length} children)
              </div>
            </div>
          </div>
          <div className={"" + maybe_hidden}>
            <CommentSection
              key={item_id}
              adapter={adapter}
              comment_ids={child_ids}
              is_visible={opened_list[idx]}
            />
          </div>
        </div>
      );
  ...
  return (
    <div
      className={
        "bg-slate-300 cursor-pointer flex flex-col w-[100%] pl-[2.5%]" +
        maybe_hidden
      }
    >
      {comment_components}
