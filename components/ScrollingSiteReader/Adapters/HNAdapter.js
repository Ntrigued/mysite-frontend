...
class HNAdapter extends AbstractAdapter {
  constructor() {
    ...
    this.top_stories_endpoint =
      "https://hacker-news.firebaseio.com/v0/topstories.json";
    this.top_stories_ids = null;
    this.basic_item_info = {};
    this.basic_item_info_endpoint =
      "https://hacker-news.firebaseio.com/v0/item/";
  }

  async getBasicItemInfo(item_id) {
    const info = await fetch(
      this.basic_item_info_endpoint + item_id + ".json"
    ).then((response) => response.json());
    if (info == null)
      throw new Error("RESPONSE_MISSING_DATA for item ", item_id);
  ...
  async getTopStoriesIDs() {
    return fetch(this.top_stories_endpoint).then((response) => response.json());
  }
  ...
  async tryForNextNItems(N) {
    ...
    return Promise.allSettled(row_promises).then((items) => {
      return items
  ...
  async getComment(item_id) {
    const comment_info = await fetch(
      this.basic_item_info_endpoint + item_id + ".json"
    ).then((response) => response.json());

    const comment_date = new Date(comment_info["time"] * 1000);
    const datetime =
      comment_date.toDateString() +
      " " +
  ...
  getDetailView(item_info) {
    ...
    if ("text" in item_info)
      content = (
        <div
          className={"my-[2.5%]"}
          dangerouslySetInnerHTML={{ __html: item_info["text"] }}
        ></div>
      );
    return (
      <div className={"flex flex-col w-full h-full overflow-y-scroll"}>
        <div className={"pl-[5%] flex flex-col"}>
          ...
          <div className={"flex flex-row justify-start min-w-[15%]"}>
            Submitted by:{" "}
            <span className={"font-bold ml-[0.5%]"}>{username}</span>
          </div>
          {content}
        </div>
        <div
          className={
            "flex flex-col flex-wrap border-t-2 hacker-news-comment-section"
          }
        >
